...
{ // ========================================== // MODELOS UNIFICADOS MONGODB // ========================================== #region Modelos [BsonIgnoreExtraElements] public class ZappingChannel { [BsonId] [BsonRepresentation(BsonType.String)] public string Id { get; set; }
...
class Program
{
            ...
            foreach (var upc in canal.Upcoming.ToList())
            {
                ...
                else
                {
                    ...
                    else if (status == "none")
                    {
                        ...
                        bool tieneFinReal = !string.IsNullOrEmpty(actualEnd);
    ...
    // ==========================================
    // MÓDULO 3: PURGA DE DESCARTADOS Y PASTS MUERTOS
    // ==========================================
    static async Task EjecutarPurgaDescartados(IMongoCollection<ZappingChannel> collection, YouTubeService yt)
    {
        ...
        var limiteAyer19 = ahora.Date.AddHours(-5); // Ayer a las 19:00 UTC (equivale a las 00:00 del día actual - 5 hs)
        ...
        foreach (var canal in canales)
        {
            ...
            // --- Limpiar Discarded (NUEVO) ---
            if (canal.Discarded != null)
            {
                foreach (var disc in canal.Discarded.ToList())
                {
                    if (DateTimeOffset.TryParse(disc.Value.PublishedAt, out var pubDate))
                    {
                        // Si la publicación original fue antes de ayer a las 19:00 UTC, ya no es necesario guardarlo
                        // porque el Worker principal también los ignora por fecha en el RSS
                        if (pubDate.ToUniversalTime() < limiteAyer19)
                        {
                            Console.WriteLine($"- {canal.ChannelName}: VOD/Reel Discarded purgado ({disc.Key}) por ser más antiguo de ayer a las 19:00 UTC.");
                            canal.Discarded.Remove(disc.Key);
                            huboCambios = true;
                        }
                    }
                }
